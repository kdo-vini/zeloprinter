...
internal static class RawPrinter
{
    ...
    public static void SendBytes(string printerName, byte[] bytes)
    {
        ...
        {
            var doc = new DocInfo();
            if (StartDocPrinter(handle, 1, doc) == 0) ThrowWin32("Não conseguimos iniciar a impressão.");
            try
            {
                ...
                if (!EndPagePrinter(handle)) ThrowWin32("Não conseguimos concluir a página de impressão.");
                if (!EndDocPrinter(handle)) ThrowWin32("Não conseguimos concluir o envio da impressão.");
            }
            catch
            {
                AbortPrinter(handle);
